...
def test_dataset_loading():
    ...
    if not val_file.exists():
        print(f"❌ FAIL: {val_file} not found")
        return False
    ...
    if not image_path.exists():
        ...
        return False
    print(f"✓ Sample image exists: {image_path}")
...
def test_dataset_class():
    ...
    try:
        ...
        if "messages" not in item:
            print("❌ FAIL: 'messages' key not found!")
            return False
        if "image" not in item:
            print("❌ FAIL: 'image' key not found!")
            return False
        ...
        try:
            ...
            print(f"❌ FAIL: Chat template failed: {e}")
            return False

        print("✅ PASS: Dataset class works\n")
        return True
...
def test_collator():
    ...
    try:
        ...
        if batch_size != 2:
            print(f"❌ FAIL: Expected batch size 2, got {batch_size}")
            return False
        ...
        if num_non_masked == 0:
            print("❌ FAIL: No target tokens in labels! Model has nothing to learn!")
            return False

        if num_non_masked < 4:  # Should have at least a few tokens per sample
            print(f"⚠️  WARNING: Very few target tokens ({num_non_masked}). Expected more.")

        print("✅ PASS: Data collator works\n")
        return True
