        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@ProfileActivity, "프로필을 불러올 수 없습니다.", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Log.e(TAG, "프로필 로드 중 오류", e)
                Toast.makeText(this@ProfileActivity, "프로필 로드 오류: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(this@ProfileActivity, "로그아웃 되었습니다.", Toast.LENGTH_SHORT).show()
                } else {
                    val errorBody = response.errorBody()?.string()
                    Log.e(TAG, "서버 로그아웃 실패: ${response.code()}, $errorBody")
                    Toast.makeText(this@ProfileActivity, "서버 로그아웃 실패, 로컬에서 로그아웃합니다.", Toast.LENGTH_SHORT).show()
                }
                ...
                Toast.makeText(this@ProfileActivity, "네트워크 오류, 로컬에서 로그아웃합니다.", Toast.LENGTH_SHORT).show()
                performLocalLogout()
            }
        }
    }
